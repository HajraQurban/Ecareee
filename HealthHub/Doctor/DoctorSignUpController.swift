...
var docimageName1 : UIImage?

class DoctorSignUpController: BaseController, UIPopoverPresentationControllerDelegate {
    ...
    lazy var DocCatTextFieldView : UIView = {
        let view = UIView()
        view.backgroundColor = UIColor.white
        view.clipsToBounds = true
        view.layer.cornerRadius = 18 * appConstant.heightRatio
    ...
    lazy var DocCattitlelbl : UILabel = {
        let lbl = UILabel()
        lbl.clipsToBounds = true
        lbl.textColor = UIColor.black
        lbl.textAlignment = .left
        lbl.font = medium(size: 11)
        lbl.text = "Select Category"
        lbl.numberOfLines = 0
        lbl.translatesAutoresizingMaskIntoConstraints = false
        return lbl
    }()
    
    lazy var DocCatTF : UITextField = {
        let tf = UITextField()
        tf.placeholder = "Dental"
        tf.backgroundColor = .clear
        tf.textColor = .black
    ...
    lazy var docbtnIcon : CustomButton = {
        let btn = CustomButton()
        btn.translatesAutoresizingMaskIntoConstraints = false
        btn.backgroundColor = UIColor.clear
    ...
    lazy var docbtnbtn : CustomButton = {
        let btn = CustomButton()
        btn.translatesAutoresizingMaskIntoConstraints = false
        btn.backgroundColor = UIColor.clear
        btn.tintColor =  UIColor(named: Colors.TextColor)
    ...
    var imgtoggle:Bool = false
    var imgtoggle1:Bool = false
    
    var listController: FPNCountryListViewController = FPNCountryListViewController(style: .grouped)
    let manager = DoctorSignUpManager()
    let dManager =  DocCategoryManagers()
    ...
    func setup()
    {
        ...
        containerView.addSubview(DocCatTextFieldView)
        containerView.addSubview(DocCattitlelbl)
        containerView.addSubview(DocCatTF)
        containerView.addSubview(docbtnIcon)
        containerView.addSubview(docbtnbtn)
        ...
        NSLayoutConstraint.activate([
            ...
            DocCatTextFieldView.topAnchor.constraint(equalTo: TypeTextFieldView.bottomAnchor, constant: 20 * appConstant.heightRatio),
            DocCatTextFieldView.leadingAnchor.constraint(equalTo: containerView.leadingAnchor, constant: 20 * appConstant.widthRatio),
            DocCatTextFieldView.trailingAnchor.constraint(equalTo: containerView.trailingAnchor, constant: -20 * appConstant.widthRatio),
            DocCatTextFieldView.heightAnchor.constraint(equalToConstant: 64 * appConstant.heightRatio),
            
            DocCattitlelbl.topAnchor.constraint(equalTo: DocCatTextFieldView.topAnchor, constant: 8 * appConstant.heightRatio),
            DocCattitlelbl.leadingAnchor.constraint(equalTo: DocCatTextFieldView.leadingAnchor, constant: 20 * appConstant.heightRatio),
            DocCattitlelbl.heightAnchor.constraint(equalToConstant: 16 * appConstant.heightRatio),
            
            DocCatTF.topAnchor.constraint(equalTo: DocCattitlelbl.bottomAnchor, constant: 5 * appConstant.heightRatio),
            DocCatTF.leadingAnchor.constraint(equalTo: DocCatTextFieldView.leadingAnchor, constant: 20 * appConstant.heightRatio),
            DocCatTF.trailingAnchor.constraint(equalTo: DocCatTextFieldView.trailingAnchor, constant: -20 * appConstant.heightRatio),
            DocCatTF.heightAnchor.constraint(equalToConstant: 20 * appConstant.heightRatio),
            
            docbtnbtn.topAnchor.constraint(equalTo: DocCatTF.bottomAnchor, constant: 0 * appConstant.heightRatio),
            docbtnbtn.leadingAnchor.constraint(equalTo: DocCatTF.leadingAnchor, constant: 0 * appConstant.heightRatio),
            docbtnbtn.trailingAnchor.constraint(equalTo: DocCatTF.trailingAnchor, constant: 0 * appConstant.heightRatio),
            docbtnbtn.bottomAnchor.constraint(equalTo: DocCatTF.bottomAnchor, constant: 0 * appConstant.heightRatio),
            
            
            docbtnIcon.centerYAnchor.constraint(equalTo: DocCatTextFieldView.centerYAnchor, constant: 0 * appConstant.heightRatio),
            docbtnIcon.trailingAnchor.constraint(equalTo: DocCatTextFieldView.trailingAnchor, constant: -12 * appConstant.widthRatio),
            docbtnIcon.heightAnchor.constraint(equalToConstant: 22 * appConstant.heightRatio),
            docbtnIcon.widthAnchor.constraint(equalToConstant: 22 * appConstant.widthRatio),
            
            
            //
            passwordTextFieldView.topAnchor.constraint(equalTo: DocCatTextFieldView.bottomAnchor, constant: 20 * appConstant.heightRatio),
...
//        ActivityIndicator.shared.showSpinner(onView: self.view)
        dManager.categorylist { docdata, msg, hasError in
...
//                    ActivityIndicator.shared.removeSpinner()
    ...
    @objc func donepressed1()
    {
        ...
        cat_id = getcatlist.data?[row].id ?? 0
//        getillnesslist.data?"".remove(at: row)
        self.DocCatTF.text = getcatlist.data?[row].name ?? ""
    ...
    @objc func docaddCatbtn_press(_ sender:UIButton){
        
        DocCatTF.placeholder = " "
        let popoverVC =  AddDoctorCategoryPopUpView()
...
//            requestoptmobileBtn.setImage(UIImage(named: "UnCheckedIcon"), for: .normal)
    ...
    func SignupRequest()
    {
        ...
        else
        {
            ...
            let parameters: [String: String] = ["name" : (nameTF.text ?? "") as String,
                                                "email" : (EmailTF.text ?? "") as String,
                                                "password" : (passwordTF.text ?? "") as String,
                                                "password_confirmation" : (confirmpasswordTF.text ?? "") as String,
                                                "contact" : (PhoneNumbTF.text ?? "") as String,
                                                "gender" : radioF as String,
                                                "address" : (addressTF.text ?? "") as String,
                                                "date_of_birth" : (DOBTF.text ?? "") as String,
...
extension DoctorSignUpController: FPNTextFieldDelegate {
...
//textfield delegates
extension DoctorSignUpController:UITextFieldDelegate{
...
extension DoctorSignUpController
{
    func validate() -> (String?, Bool) {
        ...
        let DocCattextfield = DocCatTF.text?.trimmingCharacters(in: .whitespacesAndNewlines)
...
//        }
        ...
        if DocCattextfield?.count == 0 {
            return("Doctor Category is required.", false)
        }
        
        if docimageName1 == nil {
...
extension DoctorSignUpController : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
...
extension DoctorSignUpController: DoctorCatProtocol{
    func setSym(symtoms: String, id: Int) {
        DocCatTF.text = symtoms
        cat_id = id
    }
...
extension DoctorSignUpController: UIPickerViewDelegate,UIPickerViewDataSource{
